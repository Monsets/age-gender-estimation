...
class FaceModel:
  def __init__(self, args):
    ...
    mtcnn_path = os.path.join(os.path.dirname(__file__), 'mtcnn-model_ag')
    if args.det==0:
      detector = MtcnnDetector(model_folder=mtcnn_path, ctx=ctx, num_worker=1, accurate_landmark = True, threshold=self.det_threshold)
    elif args.det == 1:
      detector = ESSHDetector(prefix='./ssh-model_ag/essh', epoch=0, ctx_id=args.gpu, test_mode=False)
      # detector = MtcnnDetector(model_folder=mtcnn_path, ctx=ctx, num_worker=1, accurate_landmark = True, threshold=[0.0,0.0,0.2])
    else:
      detector = RetinaFace('./model_ag/R50', 0, gpuid, 'net3')
